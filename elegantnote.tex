...
\begin{theorem}[Laplace 定理]
    ...
    称作代数余子式。可以证明
    \begin{equation*}
        |A|=\sum_{\substack{1 \leq j_1<...<j_k \leq n\\ j_1,...,j_k \in \{1,2,...,n\}}}
        (-1)^{i_1+i_2+...+i_k+j_1+j_2+...+j_k}
        A\left(
            \begin{aligned}
                &i_1,i_2,...,i_k\\
...
\end{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            |A|=&\sum_{\substack{1 \leq j_1<...<j_k \leq n\\ j_1,...,j_k \in \{1,2,...,n\}}}(-1)^{i_1+i_2+...+i_k+j_1+j_2+...+j_k-k(k+1)}\\
            &\times  \sum_{u_1u_2...u_k}(-1)^{\tau(u_1 u_2...u_k)}a_{i_1 u_1}a_{i_2u_2}...a_{i_k u_k}\\
            &\times   \sum_{v_1 v_2...v_k}(-1)^{\tau(v_1 v_2...v_{n-k})}a_{i_1'v_1}...a_{i_{n-k},v_{n-k}}\\
            =&\sum_{\substack{1 \leq j_1<...<j_k \leq n\\ j_1,...,j_k \in \{1,2,...,n\}}}
            A\left(
                \begin{aligned}
                    &i_1,i_2,...,i_k\\
                    &j_1,j_2,...,j_k
...
其中$A \in \mathbb{R}^{k\times k}$,$C \in \mathbb{R}^{t\times t}$,从而有
\begin{equation*}
    |D|=|A||C|
\end{equation*}

这个结果与二阶行列式的公式具有形式上的一致性。
\end{example}
