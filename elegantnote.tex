...
\begin{proof}
    ...
    \begin{equation*}
        α_j=0α_1+...+α_j+...+0\alpha_s,\quad j=1,2,...,m
    \end{equation*}
...
第三个性质的证明。
    \begin{equation*}
        \begin{aligned}
            \alpha_i=\sum_{j=1}^{r}a_{ij}\beta_{j},i=1,2,...,s\\
            \beta_j=\sum_{l=1}^{t}b_{jl}\gamma_{l},j=1,2,...,r\\
        \end{aligned}
    \end{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            \alpha_i&=\sum_{j=1}^{r}a_{ij}\sum_{l=1}^{t}b_{jl}\gamma_{l}=\sum_{j=1}^{r}\sum_{l=1}^{t}a_{ij}b_{jl}\gamma_{l}\\
            &=\sum_{l=1}^{t}\sum_{j=1}^{r}a_{ij}b_{jl}\gamma_{l}=\sum_{l=1}^{t}\gamma_{l}[\sum_{j=1}^{r}a_{ij}b_{jl}\gamma_{l}]
        \end{aligned}
    \end{equation*}
    所以有$\alpha_i$可以由$γ_1,γ_2,...,γ_t$线性表出。
\end{proof}
由以上等价关系的对称性和传递性可以得到：
\begin{proposition}
...
$\Rightarrow α_1,α_2,...,α_s$是其本身的极大线性无关组\\
$\Rightarrow \mathrm{rank}\{α_1,α_2,...,α_s\}=s$
\end{proposition}
...
\begin{proof}
    ...
    \begin{equation*}
        (\uppercase\expandafter{\romannumeral1})'\cong (\uppercase\expandafter{\romannumeral1})'\\
    \end{equation*}
    即 (\uppercase\expandafter{\romannumeral1})' 可以由  (\uppercase\expandafter{\romannumeral2})'线性表出.由推论\ref{1}，  (\uppercase\expandafter{\romannumeral1})'所含向量的个数 $\leq$ (\uppercase\expandafter{\romannumeral2})'所含向量的个数。即为
